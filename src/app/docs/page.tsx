...
async function getData() {
  try {
    const file = await fs.readFile(
      path.join(process.cwd(), "src", "content", "docs", `docs.en.mdx`),
      "utf8"
    );
...
export default async function Page() {
  ...
  const { content: mdxContent, frontmatter } = await compileMDX<{
    ...
    components: useMDXComponents({
      ...
      td: (props: any) => (
        <td className={cn("border-b px-4 py-2 text-left", props.className)}>
          {props.children}
        </td>
      ),
      pre: (props: any) => (
  ...
  return <Suspense fallback={"Loading..."}>{mdxContent}</Suspense>;
}
